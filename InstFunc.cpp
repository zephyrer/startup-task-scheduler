...
#define STR_UNINST_CPLNAME	"hi_soft_TestCPLNAME"
...
// リソースのAFX_STR_USERDAT_1 が優先される
#define STR_REGKEY_USERDAT_1	"Software\\hi soft\\hi_soft_Test"
// 上段 (この下の AFX_IDS_APP_TITLE キーを削除する)
// リソースのAFX_STR_USERDAT_2 が優先される
...
void RmUserReg(void)
{
	HKEY hCU;
	CString sProgName;	// AFX_IDS_APP_TITLE の プログラム名を読み込む
	CString sRegKey;	// AFX_STR_USERDAT_N を読み込む
	...
	// 2段階で削除する
	if(!sRegKey.LoadString(AFX_STR_USERDAT_1))
		sRegKey = STR_REGKEY_USERDAT_1;		// リソースがセットされていない時（ありえない）	
	if (RegOpenKeyEx(HKEY_CURRENT_USER, (LPCSTR)sRegKey, 0, KEY_WRITE, &hCU) == ERROR_SUCCESS)
	{
		RegDeleteKey( hCU, "Settings");
		RegCloseKey(hCU);
	}
	if(!sRegKey.LoadString(AFX_STR_USERDAT_2))
		sRegKey = STR_REGKEY_USERDAT_2;		// リソースがセットされていない時（ありえない）	
	if (RegOpenKeyEx(HKEY_CURRENT_USER, (LPCSTR)sRegKey, 0, KEY_WRITE, &hCU) == ERROR_SUCCESS)
	{
		RegDeleteKey( hCU, sProgName);
		RegCloseKey(hCU);
...
// ************************************************************
// 各種設定レジストリを削除（アンインストール時に実行）
// 配下にある全てのサブキーをスキャンし、削除する
// (HKCU)
// 2002/05/09 追加
// ************************************************************
// ユーザ領域のレジストリデータを抹消する
void RmUserRegEx(void)
{
	HKEY hCU;
	CString sProgName;	// AFX_IDS_APP_TITLE の プログラム名を読み込む
	CString sRegKey;	// AFX_STR_USERDAT_N を読み込む
	char strSubkeyName[MAX_PATH];
	int iSubkeyNameLength = MAX_PATH;
	int iSubkeyCount = 0;
	int i;
	FILETIME ftim;
	...
	// サブキーの個数を数える
	if (RegOpenKeyEx(HKEY_CURRENT_USER, (LPCSTR)sRegKey, 0, KEY_READ, &hCU) == ERROR_SUCCESS)
	{
		while(RegEnumKeyEx(hCU, iSubkeyCount, (LPSTR)strSubkeyName, (LPDWORD)&iSubkeyNameLength, NULL, NULL, NULL, (PFILETIME)&ftim) == ERROR_SUCCESS)
		{
			iSubkeyCount++;		// サブキーの個数を数える
			iSubkeyNameLength = MAX_PATH;
		}
		RegCloseKey(hCU);
	}
	// サブキーを消去
	for(i=0; i<iSubkeyCount; i++)
	{
		...
		{
			iSubkeyNameLength = MAX_PATH;
			if(RegEnumKeyEx(hCU, 0, (LPSTR)strSubkeyName, (LPDWORD)&iSubkeyNameLength, NULL, NULL, NULL, (PFILETIME)&ftim) != ERROR_SUCCESS)
			{
				break;		// エラーのときは削除中止
			}
			RegCloseKey(hCU);
		...
		if (RegOpenKeyEx(HKEY_CURRENT_USER, (LPCSTR)sRegKey, 0, KEY_WRITE, &hCU) == ERROR_SUCCESS)
		{
			RegDeleteKey( hCU, strSubkeyName);
			RegCloseKey(hCU);
		}
	}
	// プログラムのキーそのものの消去
